...
function visitWithContext(context, path) {
    ...
    return {
        ...
        ListType: function (node) {
            var fullPath = path.concat(fieldName);
            var fullPathStr = fullPath.join('.');
            var customFieldRenderer = context.resolveFieldRenderer(fullPathStr);
            var typeName = node.type.name.value;
            var type = context.resolveType(typeName);
            if (type) {
                if (customFieldRenderer.render) {
                    ...
                    invariant(false, "List Type requires a custom field renderer. No renderer found for " + fullPathStr);
                }
            }
            else {
                invariant(false, "Cannot resolve type " + typeName);
            }
...
exports.initForm = function (document, options) { return react_apollo_1.graphql(document, {
    ...
    props: function (_a) {
        ...
        if (data) {
            ...
            var name_1 = parseOperationSignature(document, 'query').name;
...
function apolloForm(document, options) {
    ...
    var withData = react_apollo_1.graphql(document, {
        ...
        props: function (_a) {
            ...
            return ({
                handleSubmit: function (variables, dispatch, props) {
                    if (mutate) {
                        return mutate(__assign({ variables: removeNotRegistredField(variables, props.registeredFields) }, options)).then(function (response) {
